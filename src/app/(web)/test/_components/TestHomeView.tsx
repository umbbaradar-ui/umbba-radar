...
// ============================================
// TestHomeView — 새 홈 미리보기 (마이레이더 존 + 엄빠레이더 추천 존)
//
// 구조 (위→아래):
//   [미리보기 안내 + 상태 스위처]
//   [가짜 검색바]
//   존 A 📡 마이레이더 (rose 그라데이션 블록, 로그인 상태별 분기)
//     1. 오늘의 레이더 브리핑 — 히어로 + 스탯 칩(앵커 스크롤)
//     2. 마감 레이더 — 관심→내 아이→전체 3단 폴백, 컴팩트 리스트
//     3. 내 키워드 새 소식 — 프리셋 키워드 실시간 매칭 데모
...
interface Props {
  ...
  viewMode: "real" | "guest" | "member";
}

// ── 키워드 매칭 (데모) ─────────────────────────
// 정식 구현 시 personalization/keyword-matching.ts 순수함수로 분리 예정.
// strong 필드(제목·브랜드·동의어)는 2자+, body는 3자+만 매칭 (오탐 방지).
...
export function TestHomeView({
  ...
  // 체크 상태 — 로그인: 서버 맵, 비로그인: localStorage (DiscoveryView와 동일 패턴)
  ...
  // 3. 키워드 셀프 — 프리셋을 최근 14일 카드에 실시간 매칭 (데모)
    ...
    <main className="mx-auto max-w-6xl px-4 py-4">
      ...
      <section className="-mx-4 rounded-b-3xl bg-gradient-to-b from-rose-50/90 via-rose-50/40 to-transparent px-4 pb-2 pt-4">
          ...
          <div id="deadline-radar" className="mb-6 scroll-mt-16">
            ...
            {!loggedIn && (
              <p className="mt-2 text-[11px] text-slate-500">
                가입하면 찜한 카드, 마감 전날 푸시로 알려드려요 · 찜해둔 카드는
                그대로 이어져요
              </p>
            )}
          </div>
        ...
        <div id="keyword-news" className="mb-6 scroll-mt-16">
          ...
          {loggedIn ? (
            <KeywordShelfDemo
              entries={keywordData.entries}
              activeEntry={activeKwEntry}
              onSelect={setSelectedKw}
              cardStatus={cardStatus}
            />
          ) : (
            <KeywordLockedTeaser entries={keywordData.entries} />
          )}
        </div>
  ...
  return (
    <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2">
      <p className="text-[11px] font-bold text-amber-800">
        🧪 새 홈 미리보기 — 실데이터 기반 테스트 페이지 (정식 반영 전)
      </p>
...
function SectionHeader({
  ...
  return (
    <div className="mb-2 flex items-center justify-between gap-2">
      <h3 className="flex items-center gap-1.5 text-sm font-extrabold tracking-tight text-slate-800">
        {title}
        {badge && (
...
/** 3. 키워드 셀프 (로그인) — 칩 선택 + 매칭 카드 레일 */
function KeywordShelfDemo({
  entries,
  activeEntry,
  onSelect,
  cardStatus,
}: {
  entries: Array<{ kw: string; matches: Post[] }>;
  activeEntry?: { kw: string; matches: Post[] };
  onSelect: (kw: string) => void;
  cardStatus: (id: string) => "applied" | "interested" | null;
}) {
  if (entries.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-200 bg-white/70 px-4 py-4">
        <p className="text-xs text-slate-500">
          기다리는 상품 있어요? 키워드를 걸어두면 뜨자마자 알려드려요.
        </p>
        <div className="mt-2 flex flex-wrap gap-1.5">
          {PRESET_KEYWORDS.slice(0, 6).map((kw) => (
            <span
              key={kw}
  ...
  return (
    <>
      <div className="mb-2 flex gap-1.5 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        <span className="shrink-0 rounded-full bg-white px-3 py-1.5 text-xs font-bold text-slate-400 ring-1 ring-dashed ring-slate-300">
          + 추가
        </span>
      </div>
      {activeEntry && activeEntry.matches.length > 0 ? (
        <CardRail>
          {activeEntry.matches.slice(0, 8).map((p) => (
            <RailCard key={p.id}>
              <PostCard post={p} status={cardStatus(p.id)} />
            </RailCard>
          ))}
        ...
        <p className="text-xs text-slate-500">
          이번 주는 조용하네요. &apos;{activeEntry?.kw}&apos; 카드가 뜨면 바로
          보여드릴게요.
        </p>
      )}
      <p className="mt-1.5 text-[10px] text-slate-400">
        ※ 데모 — 정식 반영 시 내 키워드를 직접 등록·알림받을 수 있어요
      </p>
    </>
  );
}

/** 3-변형. 키워드 잠금 티저 (비로그인) — 실매칭 건수 + 블러 카드 */
function KeywordLockedTeaser({
  entries,
}: {
  entries: Array<{ kw: string; matches: Post[] }>;
}) {
  const top = entries[0];
  return (
    <div className="relative overflow-hidden rounded-2xl border border-rose-100 bg-white/80">
      ...
      <div className="pointer-events-none flex gap-3 px-3 py-3 blur-[6px]">
        {(top?.matches ?? []).slice(0, 3).map((p) => (
          <div
            key={p.id}
            className="w-[140px] shrink-0 overflow-hidden rounded-xl bg-slate-100"
          >
            <div className="aspect-square w-full bg-slate-200" />
            <p className="line-clamp-1 px-2 py-2 text-xs font-bold text-slate-700">
              {p.title}
            </p>
          </div>
        ))}
      </div>
      <div className="absolute inset-x-0 bottom-0 flex flex-col items-center gap-1.5 bg-gradient-to-t from-white via-white/90 to-transparent px-4 pb-4 pt-8 text-center">
        <p className="text-xs font-bold text-slate-800">
          {top
            ? `'${top.kw}' 새 소식 ${top.matches.length}건이 도착해 있어요`
            : "원하는 상품 키워드를 걸어두면 뜨자마자 알려드려요"}
        </p>
        <Link
          href={top ? `/signup?kw=${encodeURIComponent(top.kw)}` : "/signup"}
