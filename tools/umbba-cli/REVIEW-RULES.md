  ...
  "items": [
    {
      ...
      "ai_confidence": 0.85,
      "from_manual_queue": true,
      "image_text_hint": "[3장] 모집기간 2026년 9월 4일 9월 20일 선정발표 10월 1일",
...
### 수동 큐 카드(`from_manual_queue: true`)와 이미지 속 마감(`image_text_hint`) — 2026-09-21 와이프
운영자가 /admin/bulk-ingest 에 **직접 고른 URL** 로 만들어진 카드다. 시스템은 이 카드를 **pass든 warn이든
마감이 캡션에서 확정돼 있으면 점수와 무관하게 바로 발행**한다(fail 만 사람이 본다). 그러니 이 카드에서
네 역할은 "실을지 말지"가 아니라 **마감이 맞는지**다:
1. 마감이 확정(`deadline_unknown: false`)이면 캡션의 기간·발표일과 대조해 틀리면 반드시 `fixes.deadline` 으로
   고쳐라(캡션 근거 필수). 맞으면 평소대로. 경품 정책·상품권 류 감점은 note 에만 적고 발행을 막으려 fail 을 주지 마라.
   **캡션에 날짜 근거가 전혀 없는데 deadline 이 채워져 있으면**(분류가 오픈·라이브·발송일을 마감으로 오인)
   `"fixes": {"deadline": "unknown"}` 으로 **마감미정으로 되돌려라** — 그래야 바로 발행되지 않고 사람이 본다.
   note 는 2번 문구로.
2. 마감미정이면 **그대로 마감미정으로 둔다**(운영자가 이미지를 보고 직접 입력해 발행). note 는 반드시 이렇게 시작:
   `캡션에 마감이 없어요 — 카드뉴스 이미지에 적혀 있을 수 있어요 → 이미지 확인 후 마감 입력해서 발행`
3. `image_text_hint` 가 있으면 수집기가 **캐러셀 이미지 속 글자(인스타 자동 인식)** 에서 뽑은 마감·기간 단서다.
   - 마감미정 카드면 note 에 **그 날짜를 그대로 인용**해라: `… → 이미지 [3장]에 '모집기간 9월 4일~9월 20일' 보임, 확인 후 9/20 로 입력해 발행`
   - **`fixes.deadline` 에는 넣지 마라** — 캡션에 없는 날짜는 서버가 버리고, 이미지 인식은 오독(0↔8, 월 누락)이 있어
     사람이 이미지로 확인하는 게 맞다. 힌트는 note 전용.
   - 힌트의 날짜가 캡션의 기간과 다르면(캡션이 우선) 힌트를 무시하고 note 에 "이미지 날짜 상이" 만 덧붙여라.
   - 힌트에 날짜가 이미 지난 것만 있으면(today_kst 기준) note 에 "이미지상 마감 지남(9/10) → 반려 검토" 로.
4. 수동 큐 카드가 아니어도(`from_manual_queue` 없음/false) 3번의 힌트 규칙은 같다 — 마감미정 note 에 인용만.
...
6. **경품이 소액 기프티콘·적립금뿐** (2026-09-15 은재·와이프) — 당첨자가 결국 받는 게 **제외 경품**뿐 (-50).
   note: `경품이 커피 기프티콘뿐이에요 → 반려 추천` (해당 유형으로 바꿔 쓰기).
   - 제외 경품: 커피/카페 음료 기프티콘 · 적립금·포인트·자사몰 쿠폰/할인권 · 편의점 모바일 상품권·올리브영 등 소액 상품권 · 베이커리·케이크·아이스크림 기프티콘
   - 인정 경품(하나라도 있으면 결격 아님, 주된 경품 기준으로 봄): 백화점 상품권 이상(신세계·롯데·현대, 상금·현금성 지원금) · 네이버페이 · 치킨 기프티콘 · 실물 제품·체험·서비스
   - 적립금으로 제품을 사야 하는 체험단/서포터즈("적립금 지급 후 직접 구매 必")는 실질이 제품 → 결격 아님
   - 표에 없는 새 유형(배민·영화관람권·문화상품권 등)이거나 판단이 애매하면 **fail 말고 warn(≤84)** + note
     `경품이 ○○ 상품권이라 정책상 제외 대상인지 확인 → 판단 후 발행` — 사람이 정한다.
7. **현장 방문형 이벤트** (2026-09-22 은재) — 참여나 경품 수령에 **팝업스토어·박람회(베페·맘앤베이비엑스포·코베 등)·
   매장/행사장 방문**이 필수 (-50). 기준은 "집에서 스마트폰만으로 응모부터 수령까지 끝나는가" — 아니면 결격.
   note: `팝업스토어에 직접 가야 참여할 수 있는 이벤트예요 → 반려 추천` (박람회 부스 방문·매장 방문·현장 수령으로 바꿔 쓰기).
   - 결격 아님: 댓글·폼·DM 응모로 끝나고 경품이 택배·모바일로 오는 이벤트 — 캡션에 팝업·박람회 홍보가 섞여 있어도 마찬가지.
     온라인·현장 응모 병행도 온라인만으로 같은 혜택이면 결격 아님.
   - 1차 분류가 놓치고 올린 방문형 카드가 오면 여기서 잡아라 — `search_keywords` 에 "박람회"·"팝업" 이 들어 있으면 방문형인지 의심.
   - **애매하면 fail 말고 warn(≤84)**: 온라인 응모인데 당첨 혜택이 오프라인 참석(팝업 초대권·오프라인 클래스/산모교실·키즈카페
     이용권·공연)이거나 박람회 사전등록 경품의 수령 방법이 불명확한 경우. note
     `당첨 혜택이 현장 참석형(○○)이라 제외 대상인지 확인 → 판단 후 발행` — 사람이 정한다.
