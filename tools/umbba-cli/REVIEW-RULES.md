...
**마감미정(`deadline_unknown: true`)은 pass 금지 — 최대 84점 warn.**
마감일을 모르는 카드는 추정값(게시일+7일)으로 발행되어 끝난 이벤트가 계속 노출되거나
살아있는 모집이 일찍 닫힌다. 사람이 날짜만 확인하면 되는 건이므로 반드시 승인 큐로 넘긴다.
(시스템도 마감미정 카드는 점수와 무관하게 자동 발행에서 제외한다 — 이중 안전장치)
...
6. `deadline_unknown: true` 카드라면, 캡션에서 "발표"·"까지"·"마감"을 **실제로 찾아본 뒤** 통과시켰나?
   근거를 찾았다면 note에 `발표 9/14 → 마감 9/13으로 보여요 → 날짜 확인 후 발행` 처럼 날짜를 적어라
   (고치는 건 사람이 한다).
