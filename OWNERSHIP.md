...
- 2026-09-12 **시기 필터 = 정확 매칭**: `/explore?stage=`·`filterPosts` 가 "그 시기 OR 전연령"으로 걸러 홈 허브 칩(임신중 21건)과 탐색 결과(107건)가 어긋났다 — 리빙이 전부 전연령이라 모든 시기 필터에 얹힘. 전연령은 "전연령" 칩·리빙 탭에서 따로. "내 아이"(my_child)·브리핑 "우리 아이 맞춤 N건"은 아직 내 시기 OR 전연령(둘이 서로 일치) — 바꾸면 마감 레이더에서 리빙 마감이 빠져 별도 결정
