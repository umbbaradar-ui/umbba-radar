...
- 2026-08-13 카드 노출 누락 제로: `selectPublishedPosts` limit 300 스톱갭 제거 → 페이지 루프(1,000행/회)로 노출 대상 발행 카드 전량 로드. Supabase 기본 Max Rows(1,000)에 조용히 잘리던 91건 복구. limit 옵션은 명시 시에만 적용(사이트맵 등)
...
- 2026-09-12 홈 ⭐추천 픽 = **앞 2장 육아 + 뒤 2장 리빙**: 육아는 기저귀·위생/외출·이동/수유·이유식/완구·교구 품목 우선·최신순, 리빙은 침구·가구/리빙·가전 우선(최신 아니어도 됨). 버킷 정렬 pinned→우선품목→마감여유(D-4+)→최신, 부족분은 반대 버킷이 채워 항상 4장. 리빙 선반은 추천 픽의 리빙 2장 제외
- 2026-09-12 시기 허브(`StageHub`) 검색바 바로 아래에도 노출(맨 아래 7번과 같은 컴포넌트, 튜토리얼 앵커는 위쪽). 모바일은 한 줄 6칸 컴팩트(아이콘+라벨, 건수 생략, 💛만) — 두 줄 136px → 49px. md 이상은 건수 표시 유지. 6칸 = 시기 5 + **🏠 리빙**(전연령 칸 대체, `/explore?topic=living`) — 리빙=전연령 단독·육아 전연령 단독 0건이라 두 집합이 같아 잃는 카드 없음. 영아/유아 겹침(62%)은 안전 마진 룰의 의도된 결과로 유지 결정(2026-09-12)
- 2026-09-12 **시기 필터 = 정확 매칭**: `/explore?stage=`·`filterPosts` 가 "그 시기 OR 전연령"으로 걸러 홈 허브 칩(임신중 21건)과 탐색 결과(107건)가 어긋났다 — 리빙이 전부 전연령이라 모든 시기 필터에 얹힘. 전연령은 "전연령" 칩·리빙 탭에서 따로. "내 아이"(my_child)·브리핑 "우리 아이 맞춤 N건"은 내 시기 OR 전연령 유지(마감 레이더와 같은 모집단) — 대신 탐색에서 **2섹션**(💛 우리 아이 시기 맞춤 N건 → 👨‍👩‍👧 전연령 신청 가능 상품도 확인하세요 N건, `SectionDivider` col-span-full). dev 전용 검수 파라미터 `/explore?preview=parent`(로그인+영아·유아 강제)
...
- 2026-09-12 **리빙 택소노미 불변식** (docs/WORKLOG-2026-09-12-LIVING-TAXONOMY.md, migration 024): 리빙(topic=living)은 ① 시기 `['all_ages']` 단독(다른 시기와 섞지 않음) ② 품목은 12종 중 리빙 5종(스킨·목욕/침구·가구/리빙·가전/식품·건강/기타)만 — 같은 키를 육아와 공유하되 topic 으로 갈라 읽는다. 단일 진실 원천 `enforceTopicTaxonomy()`(post.ts)를 어드민 폼·분류·검수·수집 모든 쓰기 경로가 거침. 검수(review-results)는 최종 topic 이 living 이면 시기·품목을 함께 맞춤. 어드민 폼은 리빙 선택 시 시기 고정·품목 5종만 노출, 승인 큐에 🏠리빙·품목 칩. 발행 데이터 42건 재분류(리빙 16 수정·육아→리빙 16 이동·마감 10 정규화). **예외(같은 날 저녁): 산모·예비맘 용품(산모 영양제·산모차·수유브라·튼살크림·젠더리빌·임신선물박스)은 어른 제품이지만 `parenting`+`['pregnancy']`** — 리빙은 전연령 단독이라 임신중 필터에서 사라지기 때문. 발행 285건 전수 스윕(워크플로우: 15장×19 분류 → 제안마다 사용주체·받는물건 2렌즈 반박 검증)으로 어른 경품(스타벅스·배민·상품권) 육아 카드 → 리빙, 산모용품 → 육아+임신중 확정 적용, 애매한 건은 사람 판단으로 남김
- 2026-09-15 **경품이 소액 기프티콘·적립금뿐인 이벤트 제외** (와이프 요청, 은재 확정): 제외 경품 = 커피·음료 기프티콘 / 적립금·포인트·자사몰 쿠폰 / 편의점·올리브영 등 소액 상품권 / 베이커리·케이크·아이스크림. 인정 경품 = 백화점 상품권 이상·상금 / 네이버페이 / 치킨 기프티콘 / 실물. 인정 경품이 하나라도 있으면 주된 경품 기준으로 살림. 표에 없거나 애매하면 skip 금지 → 검수 warn(사람 판단). RULES.md skip #11 · REVIEW-RULES 치명 결격 #6 · vision-extractor/normalizer. 기존 발행분 스윕 5건 보관(워크로그 §7)
- 2026-09-22 **현장 방문형 이벤트 제외** (은재): 팝업스토어 방문 인증·박람회(베페·맘앤베이비엑스포·코베) 부스 방문/현장 사은품/현장 추첨·매장/행사장 방문·현장 접수·현장 수령처럼 참여나 경품 수령에 특정 장소 방문이 필수면 skip. 기준 "집에서 스마트폰만으로 응모부터 수령까지 끝나는가". 온라인 응모로 끝나고 경품이 택배로 오면 홍보에 팝업·박람회가 섞여 있어도 유지. 온라인 응모+오프라인 혜택(초대권·클래스·산모교실)·수령 방법 불명확은 skip 금지 → 0.5/warn(사람 판단). RULES.md skip #12 · REVIEW-RULES 치명 결격 #7 · vision-extractor/normalizer. 기존 발행분 스윕은 미실시
- 2026-09-20 **수동 URL 큐(/admin/bulk-ingest) 맥 1시간 자동 처리** (`com.umbba.bdqueue` → `bd-queue.sh`, docs/MAC-SETUP-OPTION-C.md §5): 와이프가 넣은 게시물 URL(ingest_queue todo)은 새벽 `bd-run.sh`(계정 스캔)가 안 보던 사각지대 → 매시 정각 `bd_urls.py --from-queue`(BD 게시물 URL 직접 수집, 넣은 건수만 과금 → draft) → `bd_classify.py --ids`(그 회차 draft만) → `bd_review.py`. 큐 비면 비용 0, 새벽 루틴과 겹치면 건너뜀. 게시물 아닌 URL·비공개·이미지 없음은 failed+사유(웹 재시도 버튼). 웹은 등록만 — 분류가 맥 로컬 claude(구독)에 묶여 있어 웹 버튼을 만들어도 pending 시점이 같아서 맥 편입으로 결정
